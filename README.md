...
> Modified from [Original Ent Proto](https://github.com/ent/contrib/tree/master/entproto)
...
```console
go get -u github.com/yoshino-s/entproto
```

Install `protoc-gen-entgrpc` (ent's gRPC service implementation generator):

```console
go get github.com/yoshino-s/entproto/cmd/protoc-gen-entgrpc
```
...
```console
go run github.com/yoshino-s/entproto/cmd/entproto -path ./ent/schema
```
...
option go_package = "github.com/yoshino-s/entproto/internal/todo/ent/proto/entpb";
...
go generate ./cmd/protoc-gen-entgrpc/... &&
  go install github.com/yoshino-s/entproto/cmd/protoc-gen-entgrpc &&
  go generate ./... &&
  go test ./...
```
